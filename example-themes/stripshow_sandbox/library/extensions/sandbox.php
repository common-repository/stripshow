<?php
/**
* This file contains functions that were brought over from Sandbox.
* These functions have for the most part remained in their original
* forms. Any function may become deprecated as I customize and rename
* functions to better suit the needs of stripShow.
*/

// Produces a list of pages in the header without whitespace
function sandbox_globalnav() {
	if ( $menu = str_replace( array( "\r", "\n", "\t" ), '', wp_list_pages('title_li=&sort_column=menu_order&echo=0') ) )
		$menu = '<ul class="sf-menu sf-js-enabled">' . $menu . '</ul>';
...
function sandbox_gallery($attr) {
	...
	$total_images = sizeof($attachments);
	$i = 0;
	foreach ( $attachments as $id => $attachment ) {
		...
		$output .= "\n\t" . '</' . $itemtag . '>';
		$i++;
		if ( $columns > 0 && $i % $columns == 0 && $i < $total_images)
			$output .= "\n</div>\n" . '<div class="gallery">';
	}
	$output .= "\n</div>\n";
...
function widget_sandbox_search($args) {
	...
	$title = empty($options['title']) ? __( 'Search', 'stripshow' ) : attribute_escape($options['title']);
	$button = empty($options['button']) ? __( 'Find', 'stripshow' ) : attribute_escape($options['button']);
?>
...
?>
	<p><label for="search-title"><?php _e( 'Title:', 'stripshow' ) ?> <input class="widefat" id="search-title" name="search-title" type="text" value="<?php echo $title; ?>" /></label></p>
	<p><label for="search-button"><?php _e( 'Button Text:', 'stripshow' ) ?> <input class="widefat" id="search-button" name="search-button" type="text" value="<?php echo $button; ?>" /></label></p>
...
function widget_sandbox_meta($args) {
	...
	$title = empty($options['title']) ? __( 'Meta', 'stripshow' ) : attribute_escape($options['title']);
?>
			<?php echo $before_widget; ?>
...
function widget_sandbox_rsslinks($args) {
	...
	$title = empty($options['title']) ? __( 'RSS Links', 'stripshow' ) : attribute_escape($options['title']);
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
			<ul>
				<li><a href="<?php bloginfo('rss2_url') ?>" title="<?php echo wp_specialchars( get_bloginfo('name'), 1 ) ?> <?php _e( 'Posts RSS feed', 'stripshow' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All posts', 'stripshow' ) ?></a></li>
				<li><a href="<?php bloginfo('comments_rss2_url') ?>" title="<?php echo wp_specialchars(bloginfo('name'), 1) ?> <?php _e( 'Comments RSS feed', 'stripshow' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All comments', 'stripshow' ) ?></a></li>
			</ul>
		<?php echo $after_widget; ?>
<?php
}
...
function widget_sandbox_rsslinks_control() {
	...
	$title = attribute_escape($options['title']);
?>
	<p><label for="rsslinks-title"><?php _e( 'Title:', 'stripshow' ) ?> <input class="widefat" id="rsslinks-title" name="rsslinks-title" type="text" value="<?php echo $title; ?>" /></label></p>
...
function sandbox_widgets_init() {
	...
	wp_register_sidebar_widget( 'search', __( 'Search', 'stripshow' ), 'widget_sandbox_search', $widget_ops );
	unregister_widget_control('search'); // We're being Sandbox-specific; remove WP default
	wp_register_widget_control( 'search', __( 'Search', 'stripshow' ), 'widget_sandbox_search_control' );
	...
	//wp_register_sidebar_widget( 'meta', __( 'Meta', 'stripshow' ), 'widget_sandbox_meta', $widget_ops );
	//unregister_widget_control('meta'); // We're being Sandbox-specific; remove WP default
	//wp_register_widget_control( 'meta', __( 'Meta', 'stripshow' ), 'wp_widget_meta_control' );
	...
	wp_register_sidebar_widget( 'rss_links', __( 'RSS Links', 'stripshow' ), 'widget_sandbox_rsslinks', $widget_ops );
	wp_register_widget_control( 'rss_links', __( 'RSS Links', 'stripshow' ), 'widget_sandbox_rsslinks_control' );
}

// Translate, if applicable
load_theme_textdomain('stripshow');
...
function sandbox_comments($comment, $args, $depth) {
    ...
    ?>
    	<li id="comment-<?php comment_ID() ?>" class="<?php sandbox_comment_class() ?>">
    		...
    		<div class="comment-meta"><?php printf(__('Posted %1$s at %2$s <span class="meta-sep">|</span> <a href="%3$s" title="Permalink to this comment">Permalink</a>', 'stripshow'),
    					get_comment_date(),
    					get_comment_time(),
    					'#comment-' . get_comment_ID() );
    					edit_comment_link(__('Edit', 'stripshow'), ' <span class="meta-sep">|</span> <span class="edit-link">', '</span>'); ?></div>
			...
			<?php // echo the comment reply link with help from Justin Tadlock http://justintadlock.com/ and Will Norris http://willnorris.com/
				if($args['type'] == 'all' || get_comment_type() == 'comment') :
					comment_reply_link(array_merge($args, array(
						'reply_text' => __('Reply','stripshow'), 
						'login_text' => __('Log in to reply.','stripshow'),
						'depth' => $depth,
...
function sandbox_pings($comment, $args, $depth) {
       $GLOBALS['comment'] = $comment;
        ?>
    		<li id="comment-<?php comment_ID() ?>" class="<?php sandbox_comment_class() ?>">
    			<div class="comment-author"><?php printf(__('By %1$s on %2$s at %3$s', 'stripshow'),
